class Filepath
  ...
  attr_accessor :realpath,:html_name, :yml_name
  def initialize(f=__FILE__)
    ...
    @html_name = this_script_dir + %{/output/} + this_script_basename + %[.html] # erp_user_priv.html
    @yml_name = this_script_dir + %{/config/} + %[database.yml] # erp_user_priv.yml
  end
